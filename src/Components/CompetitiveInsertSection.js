...
class CompetitiveInsertSection extends React.Component {
  ...
  handleWinLoseSelect = index => {
    this.setState({
      Result: results[index]
    });
  };

  handleRankSelect = index => {
    this.setState({
      Rank: rank[index]
    });
  };

  handleHeroSelect = index => {
    this.setState({
      Character: heroes[index]
    });
  };

  handleMapSelect = index => {
    this.setState({
      Map: maps[index]
    });
  };
  ...
  handleInsert = func => {
    ...
    if (
      this.state.seasonError === "" &&
      this.state.seasonError === "" &&
      this.state.seasonError === ""
    ) {
      axios
        .post("http://localhost:3001/competitive", newRecord)
  ...
  render() {
    heroes = this.props.heroes.map(index => index.Name);

    maps = this.props.maps.map(index => index.Name);
      ...
      <MuiThemeProvider>
        ...
        >
          <SelectField
            ...
            onChange={(evt, newIndex) => this.handleHeroSelect(newIndex)}
          ...
          <SelectField
            ...
            onChange={(evt, newIndex) => this.handleMapSelect(newIndex)}
          ...
          <SelectField
            ...
            onChange={(evt, newIndex) => this.handleWinLoseSelect(newIndex)}
          ...
          <TextField
            ...
            onChange={(event, newValue) => {
              if (newValue && parseInt(newValue) == newValue) {
                ...
                this.setState({
                  seasonError: "This field has to be a number!"
                });
              }
            }}
          />
          ...
          <SelectField
            ...
            onChange={(evt, newIndex) => this.handleRankSelect(newIndex)}
          ...
          <TextField
            ...
            onChange={(event, newValue) => {
              if (parseInt(newValue) == newValue) {
                ...
                this.setState({
                  srDiffError: "This field has to be a number!"
                });
              }
            }}
          />
          ...
          <RaisedButton
            ...
            style={style}
            onClick={() =>
              this.handleInsert(this.props.handleNewCompetitiveRecord)
            }
            disabled={isDisabled}
          />
        </form>
