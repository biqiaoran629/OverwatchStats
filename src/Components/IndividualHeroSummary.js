...
const RowItem = param => {
  return (
    <TableRow justify="between" separator="horizontal">
      <td>{param.columnName}</td>
      <td>{(param.heroWinPercentage * 100).toFixed(2)}%</td>
      <td>
        {param.heroWin} - {param.heroLoss}
      </td>
      <td>{(param.totalWinPercentage * 100).toFixed(2)}%</td>
      <td>
...
class IndividualHeroSummary extends React.Component {
  ...
  componentDidMount() {
    axios
      .all([
        axios.post(
          `http://localhost:3001/quickplay/stats/` + this.props.match.params.id
        ),
        axios.post(`http://localhost:3001/quickplay/stats`)
      ])
      .then(
        axios.spread((heroData, totalData) => {
          this.setState({
  ...
  createImage = (image, history) => {
    const imgSrc = require("../css/images/Icon-" +
      image.toLowerCase() +
      ".png");
    return (
      <Box pad="large" justify="center" alignContent="center">
        <img
          src={imgSrc}
          width={"100px"}
          alt={image}
